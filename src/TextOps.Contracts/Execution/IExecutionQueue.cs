...
public interface IExecutionQueue : IExecutionDispatcher
{
    ...
    /// </summary>
    Task<QueuedDispatch?> ClaimNextAsync(string workerId, CancellationToken ct = default);
    
    /// <summary>
    /// Marks a claimed dispatch as completed (success or failure).
    /// </summary>
    Task CompleteAsync(long queueId, bool success, string? error, CancellationToken ct = default);
    
    /// <summary>
    /// Releases a claimed dispatch back to pending (for retry).
    /// </summary>
    Task ReleaseAsync(long queueId, string? error, CancellationToken ct = default);
    
    /// <summary>
    /// Reclaims entries that have been locked longer than the timeout.
    /// Called periodically to handle worker crashes.
    /// </summary>
    Task<int> ReclaimStaleAsync(TimeSpan lockTimeout, CancellationToken ct = default);
}
